...
y = zeros(K, N_BLK, N_SNR, N_AMP, N_A0, N_MC2);
BER = zeros(K, N_SNR, N_AMP, N_A0, N_MC2);

% Posição de usuários fixa por simulação
[x_user, y_user] = userPositionGenerator(K, radial);
    ...
    for snr_idx = 1:N_SNR
        for a_idx = 1:N_A0
            for amp_idx = 1:N_AMP
                ...
                y(:,:,snr_idx, amp_idx, a_idx, mc_idx2) = H.' * amplifier(sqrt(snr(snr_idx)) * x_normalized, current_amp_type, a0) + v_normalized;
                bit_received = zeros(B * N_BLK, K);

                for users_idx = 1:K
                    s_received = y(users_idx, :, snr_idx, amp_idx, a_idx, mc_idx2).';
                    ...
                    BER(users_idx, snr_idx, amp_idx, a_idx, mc_idx2) = bit_error;
                end
            end
        end
    end
...
save('ber_mc_zf_ssf.mat', 'M', 'K', 'y', 'SNR', 'BER', 'N_AMP', 'N_A0', 'A0', 'precoder_type', 'amplifiers_type', 'x_user', 'y_user');